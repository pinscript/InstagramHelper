chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
	if (request.action === "get_followers") {
		chrome.runtime.sendMessage({ action : "return_followers", text : document.getElementsByClassName("_539vh")[0].innerHTML });
	...
	//	chrome.runtime.sendMessage({ action : "return_followers_count", text : window._sharedData.entry_data.ProfilePage[0].user.followed_by.count });
        ...
        delete (elem);
        var a = ret.callResult.entry_data.ProfilePage[0].user.followed_by.count;
		chrome.runtime.sendMessage({ action : "return_followers_count", text : a });
	}
});
